...
func TestRouter_On_UnmatchedMessageCallsFallback(t *testing.T) {
	defaultFallbackCalled := false
	customFallback := func(ctx *router.Context) {
		if ctx.Message.Event == "unknown" {
	...
	rtr.Dispatch(newMockConnection("c1", "r1"), wspulse.Message{Event: "unknown"})

	assert.True(t, defaultFallbackCalled, "fallback was not called for unmatched message event")
}
...
func TestRouter_On_DispatchRoutesCorrectEvent(t *testing.T) {
	...
	rtr.Dispatch(newMockConnection("c1", "r1"), wspulse.Message{Event: "chat"})
...
func TestRouter_WithFallback_CalledForUnregisteredEvent(t *testing.T) {
	...
	rtr := router.New(router.WithFallback(func(ctx *router.Context) {
		gotEvent = ctx.Message.Event
	}))

	rtr.Dispatch(newMockConnection("c1", "r1"), wspulse.Message{Event: "mystery"})
...
func TestRouter_DefaultFallback_CalledForUnmatchedEventWithNoCustomFallback(t *testing.T) {
...
func BenchmarkDispatch(b *testing.B) {
	...
	msg := wspulse.Message{Event: "ping"}
